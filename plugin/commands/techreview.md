...
All three must exist. If any are missing, tell the user which commands to run first (`/product-kit:solution`, `/product-kit:prioritize`, `/product-kit:spec`).
...
Read `knowledge-index.md` if it exists — it contains a summary of research from the `knowledge/` directory. Reference relevant findings when assessing feasibility. If the file doesn't exist but `knowledge/` has files, suggest running `/product-kit:learn` first.
